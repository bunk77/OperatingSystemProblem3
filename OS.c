                        ...
                        switch (call) {
                            case CODE_LOCK:
                                ...
                                if (FIFOq_peek(group[current->group]->fmutex[resource], error) == current)
                                    lock = true;
                                else if (!FIFOq_is_empty(group[current->group]->fmutex[resource], error)) {
...
{   
    ...
    if (t < 0) {
        ...
        if ((OUTPUT || MUTEX_DEBUG) && current->resource == -1)
        ...
        if (FIFOq_peek(group[current->group]->fcond[t], error) != NULL)
            pcb = FIFOq_dequeue(group[current->group]->fcond[t], error);
        else
            printf("bad signal\n");
        if (pcb != NULL) {
            FIFOq_enqueuePCB(group[pcb->group]->fmutex[pcb->resource], pcb, error);
            if (FIFOq_peek(group[pcb->group]->fmutex[pcb->resource], error) == pcb)
                newlock = true;
            pcb->resource = -1;
        }
        if (OUTPUT || MUTEX_DEBUG)
...
//                        FIFOq_dequeue(group[t]->fmutex[r]->waiting, NULL), error);
                //if (!FIFOq_is_empty(group[t]->fmutex[r]->waiting, error))
                queueCleanup(group[t]->fmutex[r], mQ, error);
                //else
                    //FIFOq_destruct(mutexQ, error);
